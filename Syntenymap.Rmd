...
plot.synteny.mean <- function(synteny_stats) { 
  ...
  ggplot(tidy, aes(x = SPECIES, y = RESULT, fill = SPECIES)) + 
    geom_bar(stat = "identity",
             color = "#325d81", size = 1, fill = "#edf3f8" ) +
    labs(x = "\nComparison Species Genome", y = "Total Number/Length\n") +
    facet_wrap(~ STAT, scales = "free") +
    theme_standard 
} 
...
Plot mean number of loci (max/min) per synteny block per species.
...
plot.synteny.loci <- function(synteny_stats) { 
  ...
  ggplot(tidy, aes(x = SPECIES, y = RESULT)) + 
    geom_point(data = filter(tidy, STAT == "MEAN_LOCI"),
...
plot.synteny.loci(synteny_stats)

```

Plot mean, min and max length of synteny blocks per species.
...
plot.synteny.bp <- function(synteny_stats) { 
  ...
  # plot
  ggplot(tidy, aes(x = SPECIES, y = RESULT)) + 
    geom_point(data = filter(tidy, STAT == "MEAN_BP"),
             shape = 23, size = 3, fill = "black") +
    geom_point(data = filter(tidy, STAT == "MIN_BP"),
               shape = 21, size = 2, fill = "black") +
    geom_point(data = filter(tidy, STAT == "MAX_BP"),
               shape = 21, size = 2, fill = "black") +
    labs(x = "Species genome", y = "Mean length blocks (Max/Min) in bp") +
    theme_standard
} 

plot.synteny.bp(synteny_stats)
...
plot.synteny.cM <- function(synteny_stats) { 
  ...
  # plot
  ggplot(tidy, aes(x = SPECIES, y = RESULT, fill = SPECIES)) + 
    geom_point(data = filter(tidy, STAT == "MEAN_cM"),
             shape = 23, size = 3, fill = "black") +
    geom_point(data = filter(tidy, STAT == "MIN_cM"),
               shape = 21, size = 2, fill = "black") +
    geom_point(data = filter(tidy, STAT == "MAX_cM"),
               shape = 21, size = 2, fill = "black") +
    labs(x = "Species genome", y = "Mean length blocks in cM (Max/Min)") +
    theme_standard
} 

plot.synteny.cM(synteny_stats)
  ...
  lapply(species, function(species){
    ...
    ggplot(plot, aes(x = MAP_LG, y = COMP_CHR)) +
      geom_point(aes(size = n), 
                 shape = 21, stroke = 1.1, color = "#325d81", fill = "#edf3f8") +
