...
Plot linear map of male and female-specific maps with marker clusters re-inserted.

### female map

```{r plot female map, fig.height=6, fig.width=10, message=FALSE, warning=FALSE}

# read map data
FamB_F.map <- read.table("results/FamB_F.map", 
                         header = TRUE, stringsAsFactors = FALSE)

# plot with ggplot
ggplot(FamB_F.map, aes(x = LG_F, y = POS_F)) +
  geom_path(aes(group = LG_F), size = 9, lineend = "round", col = "black") +
  geom_path(aes(group = LG_F), size = 7.5, lineend = "round", col = "grey95") +
  geom_rect(aes(xmin = LG_F-0.35, xmax = LG_F+0.35, 
                ymin = POS_F-0.025, ymax = POS_F+0.025),
  ...
  labs(x = "\nLinkage Group Fam B female", y = "Position (cM)\n") +
  theme_classic() +
...
```{r plot male map, fig.height=6, fig.width=10, message=FALSE, warning=FALSE}

# read map data
FamB_M.map <- read.table("results/FamB_M.map", 
                         header = TRUE, stringsAsFactors = FALSE)

# plot with ggplot
ggplot(FamB_M.map, aes(x = LG_M, y = POS_M)) +
  geom_path(aes(group = LG_M), size = 9, lineend = "round", col = "black") +
  geom_path(aes(group = LG_M), size = 7.5, lineend = "round", col = "grey95") +
  geom_rect(aes(xmin = LG_M-0.35, xmax = LG_M+0.35, 
                ymin = POS_M-0.025, ymax = POS_M+0.025),
            color = "black", fill = "black") +
  scale_x_continuous(breaks = c(1:24)) +
  scale_y_continuous(breaks = c(0, 10, 20, 30, 40, 50, 60, 70, 80)) +
  # scale_Mill_manual(name="Marker Type",
  #                   breaks=c("est", "msat", "snp"),
  #                   labels=c("EST-SSR", "Microsatellite", "RAD Locus"),
  #                   values=c('#9e0142', '#a1d99b', '#2b8cbe') +
  labs(x = "\nLinkage Group Fam B male", y = "Position (cM)\n") +
  theme_classic() +
  theme(axis.title = element_text(size = 16),
        axis.title.y = element_text(vjust = 1.5),
        axis.line.x = element_blank(),
...
# read map data
FamB.map <- read.table("results/FamB_family.map", 
                         header = TRUE, stringsAsFactors = FALSE)
...
ggplot(FamB.map, aes(x = LG, y = POSITION)) +
  ...
  #                   labels=c("EST-SSR", "Microsatellite", "RAD Locus"),
  #                   values=c('#9e0142', '#a1d99b', '#2b8cbe') +
  labs(x = "\nLinkage Group Fam B", y = "Position (cM)\n") +
  theme_classic() +
  theme(axis.title = element_text(size = 16),
        axis.title.y = element_text(vjust = 1.5),
        axis.line.x = element_blank(),
