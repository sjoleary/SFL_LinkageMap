...
Species_comp <- data.frame(Species = c("gacu", "tnig", "trub", "onil", "lcal", "dlab", "poli", "csem"),
...
species <- c("csem", "dlab", "gacu", "lcal", "onil", "poli", "tnig", "trub") 
...
SPECIES <- c("csem", "dlab", "gacu", "lcal", "onil", "poli", "tnig", "trub") 
GENOME <- c(470, 578, 463, 586, 657, 643, 240, 281)

genome <- data.frame(SPECIES, GENOME) %>%
    arrange(SPECIES)

synteny_stats <- left_join(synteny_stats, genome) %>%
    mutate(GENOME = GENOME*1000000,
           GENOME_COV = TOTAL_BP/GENOME*100,
           LINKMAP = 1605.43,
           LINKMAP_COV = TOTAL_cM/LINKMAP*100)

# plot genome size vs coverage
synteny_stats %>%
    select(GENOME, GENOME_COV, LINKMAP_COV) %>%
    gather(key = COVERAGE, value = PERCENT, 2:3) %>%
...
tidy <- synteny_stats %>% 
    gather("STAT", "RESULT", 2:18) %>%
    filter(STAT %in% c("MEAN_LOCI", "GENOME_COV", "LINKMAP_COV"))
...
temp <- synteny_stats %>%
    select(SPECIES, GENOME, N_LOCI, N_BLOCKS, MEAN_LOCI, TOTAL_BP, TOTAL_cM, GENOME_COV, LINKMAP_COV) %>%
    gather("STAT", "RESULT", 3:9)
...
Comparison relations ships number of blocks
...
plot.oxford(synt_blocks, c("csem", "dlab", "lcal", "gacu", "onil", "poli", "tnig", "trub"))
