...
tidy <- synteny_stats %>% 
    gather("STAT", "RESULT", 2:14) %>%
    filter(STAT %in% c("N_LOCI", "N_BLOCKS", 
                       "TOTAL_BP", "TOTAL_cM", 
                       "MEAN_BP", "MEAN_cM",
                       "MEAN_LOCI"))

SPECIES <- c("dlab", "gacu", "lcal", "onil", "tnig", "trub") 
STAT <- c("%-genome", "%-genome", "%-genome", "%-genome", "%-genome", "%-genome")
RESULT <- c(49.9, 34.0, 48.3, 40.6, 37.7, 38.5)

coverage_genome <- data.frame(SPECIES, STAT, RESULT)

temp <- bind_rows(tidy, coverage_genome)

SPCIES <- c("dlab", "gacu", "lcal", "onil", "tnig", "trub") 
STAT <- c("%-linkmap", "%-linkmap", "%-linkmap", "%-linkmap", "%-linkmap", "%-linkmap")
RESULT <- c(38.6, 21.9, 47.7, 7.6, 3.5, 4.9)

coverage_linkmap <- data.frame(SPECIES, STAT, RESULT)

temp <- bind_rows(temp, coverage_linkmap)
...
ggplot(temp, aes(x = SPECIES, y = RESULT, fill = SPECIES)) + 
    geom_bar(stat = "identity",
             color = "black", size = 0.7, fill = "grey75" ) +
    labs(x = "\nComparison Species Genome", y = "mean number loci | length (cM/bp) | %-coverage\n") +
    facet_wrap(~ STAT, scales = "free") +
    theme_facet
