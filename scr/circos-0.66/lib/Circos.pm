...
sub run {
	...
	for my $ideogram (@IDEOGRAMS) {
		...
		for my $svgparam_hash (seek_parameter_glob("^svg.*",undef,fetch_conf("ideogram"))) {
			for my $svgparam (keys %$svgparam_hash) {
				...
				if ( $value =~ /^eval\(\s*(.*)\s*\)\s*$/ ) {
					my $expr = $1;
					$value = Circos::Expression::eval_expression( {data=>[$ideogram]}, $expr );
				} else {
				}
	...
	for my $track ( @plot_tracks ) {
		...
		if ( defined $track_type && $track_type eq "text" ) {
	...
	for my $track ( sort {$a->{z} <=> $b->{z}} @plot_tracks ) {
		...
		if ( defined $track_type && $track_type =~ /scatter|line|histogram|heatmap/ && ( !defined $plot_min || !defined $plot_max ) ) {
	    my @values;
		...
		my $f   = $sort_funcs->{$track_type} if defined $track_type;
		$f    ||= $sort_funcs->{default};
